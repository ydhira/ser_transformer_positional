...
class TransformerInterface(nn.Module):
    ...
    activation: torch.nn.Module, optional
        ...
        e.g., relu or gelu or attention.swish.
    ...
    conformer_activation: torch.nn.Module, optional
        Activation module used after Conformer convolutional layers. E.g. attention.Swish, ReLU etc. it has to be a torch Module.
    ...
    def __init__(
        ...
        if positional_encoding == "fixed_abs_sine":
            self.positional_encoding = PositionalEncoding(d_pos_enc, max_length)
            self.positional_encoding2 = PositionalEncoding(d_model, max_length)
        ...
        if num_encoder_layers > 0:
            ...
            print("Initializing :", encoder_module)
            if encoder_module == "transformer":
                self.encoder = TransformerEncoder(
                    nhead=nhead,
                    num_layers=num_encoder_layers,
                    d_ffn=d_ffn,
                    d_model=d_pos_enc, ### CHANGED THIS 
                    dropout=dropout,
                ...
                self.encoder = CNNEncoder(d_model=d_model, d_pos_enc=d_pos_enc)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = attention.LayerNorm(d_model, eps=1e-6)
        self.norm2 = attention.LayerNorm(d_model, eps=1e-6)
    ...
    def forward(
        ...
        # print("2: ", output.shape,self_attn.shape )
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.norm = attention.LayerNorm(d_model, eps=1e-6)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if attention_type == "regularMHA":
            ...
            self.mutihead_attn = attention.MultiheadAttention(
                nhead=nhead,
                d_model=d_model,
                kdim=kdim,
                vdim=vdim,
            ...
            self.self_attn = attention.RelPosMHAXL(
                d_model, nhead, dropout, mask_pos_future=causal
            )
            self.mutihead_attn = attention.RelPosMHAXL(
                d_model, nhead, dropout, mask_pos_future=causal
            )
        ...
        self.norm1 = attention.LayerNorm(d_model, eps=1e-6)
        self.norm2 = attention.LayerNorm(d_model, eps=1e-6)
        self.norm3 = attention.LayerNorm(d_model, eps=1e-6)
        self.dropout1 = torch.nn.Dropout(dropout)
        self.dropout2 = torch.nn.Dropout(dropout)
...
class CNNEncoder(nn.Module):
    ...
    def __init__(self, d_model, d_pos_enc):
        super(CNNEncoder, self).__init__()
        self.d_model = d_model
        self.convlayers = nn.Sequential(
                        nn.Conv2d(1, d_model, 3, 2, bias=False),
                        nn.BatchNorm2d(d_model),
                        nn.ReLU(inplace=True),
                        nn.Conv2d(d_model, d_model, 3, 2, bias=False),
                        )

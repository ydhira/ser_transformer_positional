...
class TransformerASR(TransformerInterface):
    ...
    def forward(self, src, tgt, wav_len=None, pad_idx=0):
        ...
        if self.attention_type == "RelPosMHAXL":
            ...
            import pdb 
            pdb.set_trace()
            print("Source positional encoding shape: ", self.positional_encoding(src).shape)
        ...
        if self.attention_type == "RelPosMHAXL":
            ...
            print("Target positional encoding shape: ", self.positional_encoding(tgt).shape)
        ...
        # decoder_out_r = decoder_out.reshape(decoder_out.shape[0], -1)
        decoder_out_r = decoder_out.transpose(2,1)
        decoder_out_r = nn.functional.avg_pool1d(decoder_out_r, [decoder_out_r.size()[2]], stride=1).squeeze()
        # print(decoder_out_r.shape)
        logits = self.fclayer(decoder_out_r)
        # print("Logits: ", logits.shape)
        logits =  nn.functional.log_softmax(logits, dim=-1)
        return encoder_out, decoder_out, logits
